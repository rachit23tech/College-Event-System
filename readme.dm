...
A centralized, easy-to-use web application for colleges to create, manage, and promote campus events. Students and staff can discover, register for, and attend events, while administrators get analytics and tools to optimize engagement and logistics.
 
## Business Goal
 
Provide a centralized, easy-to-use web application for colleges to create, manage, and promote campus events; enable students and staff to discover, register for, and attend events; and give administrators analytics and tools to optimize engagement and logistics.
...
**EM-01** — As an Organizer, I want to create an event with title, description, date/time, location, and images so that students can discover it.
...
**EM-02** — As an Organizer, I want to create recurring events so attendees can sign up for repeating sessions.
...
**UA-01** — As a Student, I want to sign up and log in with my institutional email so I can register for events.
- Implement authentication (email/password, SSO)
- Build registration and login flows
- Add profile edit page and avatar upload
**UA-02** — As an Admin, I want to manage user roles so I can grant organizer or staff privileges.
- Implement role management UI
- Enforce permissions server-side for sensitive actions
### Epic: Registration & Ticketing
 
**RT-01** — As a Student, I want to register for an event and receive a ticket/QR code so I can check in at the door.
...
**RT-02** — As an Organizer, I want to set capacity and waitlist so event capacity is enforced.
...
**NC-01** — As an Organizer, I want to send announcements to registered attendees so they get timely updates.
...
**AD-01** — As an Admin, I want to see registration and attendance analytics so I can measure event success.
- Design dashboard pages for key metrics
- Implement backend aggregation queries
- Add CSV export for reports
### Epic: Integrations & APIs
 
**API-01** — As an external system, I want to fetch public event data via API so I can display it elsewhere.
- Design and implement public REST endpoints
- Add pagination, filtering, and caching
### Epic: Security & Compliance
 
**SC-01** — As a System Admin, I want audit logs for admin actions so I can investigate issues.
- Implement audit logging for critical actions
- Create UI to review audit logs (with filters)
### Epic: Mobile & Accessibility
 
**MA-01** — As a User with a phone, I want a responsive site so I can register on the go.
...
1. Clone the repo
2. Install dependencies *(add language-specific instructions to this section)*
3. Set up local environment variables (database, email, payment keys)
4. Run database migrations and seed sample data
5. Start the dev server and log in with a seed account
## Notes
